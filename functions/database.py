...
def insert_data_into_ebay(dataframe):
    ...
    for index, row in dataframe.iterrows():
        cur.execute("INSERT INTO ebay (category, title, price, urlItem, urlImage) values(%s, %s, %s, %s, %s)", (row.category, row.title, row.price, row.urlItem, row.urlImage))
...
def join_to_csv():
    ...
    cur.execute("select categories.category, ebay.title, ebay.price, ebay.urlItem, ebay.urlImage from ebay LEFT JOIN categories on ebay.category = categories.id")
